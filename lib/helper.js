...
export const getAll = (Model, status) =>
    catchAsync(async (req, res) => {
        ...
        const query = Model.find(queryParms).sort({ createdAt: -1 })
...
export const getTokenFromCampay = async (creds) => {
    try {
        const response = await fetch(`${process.env.CAMPAY_BASE_URL}/token/`, {
            method: 'post',
            headers: {
